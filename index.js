...
class OheaderChange {
  ...
  run(lovelace = this.main.querySelector("partial-panel-resolver").querySelector("ha-panel-lovelace")) {
    ...
    if (!lovelace||window.location.href.includes("?disableOheader")) {
      ...
      console.log("OHassHeaderPositionCard disabled");
      return;
    }
          ...
          requestAnimationFrame(() => {
            setTimeout(() => {
              ...
              if (urlSplit[1] === this.panelUrl) {
                ...
                // 处理滚动逻辑
                if (newViewIndex !== -1) {
                  if (this.viewIndex === newViewIndex) {
                    // 同一页面，滚动到顶部
                    document.documentElement.scrollTop = 0;
                  } else {
                    // 不同页面，恢复滚动位置
                    document.documentElement.scrollTop = this.scrollTopArr[newViewIndex] || 0;
                  }
